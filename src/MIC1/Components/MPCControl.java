...
public class MPCControl
{
    ...
    public MPCControl(MIR _mir, ALU _alu, DataPath _dataPath)
    {
        ...
        else
        {
            boolean N = _alu.getN();
            boolean Z = _alu.getZ();
            boolean m1 = MPC.charAt(0) == 1 ? true : false;
            boolean F = (Z && nowInstrucion.charAt(11) == '1' ? true : false) || (N && nowInstrucion.charAt(10) == '1' ? true : false) || m1;
            MPCArr[0] = F ? '1' : '0';
        }
        if (mir.getInstruction().charAt(9) == '1')
        {
            ...
            for (int i = 1; i < 9; i++)
                MPCArr[i] = (MBRarr[i - 1] || (MPCArr[i] == '1' ? true : false) ? '1' : '0');
        }
